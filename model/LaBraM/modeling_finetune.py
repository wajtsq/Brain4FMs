...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
                 ...
                 use_mean_pooling=True, init_scale=0.001, seq_len=16, **kwargs):
        super().__init__()
        ...
        if use_abs_pos_emb:
            ...
            self.pos_embed = None
        self.time_embed = nn.Parameter(torch.zeros(1, seq_len, embed_dim), requires_grad=True)
    ...
    def forward_features(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        if self.pos_embed is not None:
            ch_num = (x.shape[1]-1) // input_time_window
            pos_embed_used = self.pos_embed[:, input_chans] if input_chans is not None else self.pos_embed[:, :ch_num+1]
