...
from model.LaBraM.modeling_finetune import NeuralTransformer
from model.LaBraM.norm_ema_quantizer import NormEMAVectorQuantizer
...
class VQNSP(nn.Module):
    def __init__(self,
        ...
        encoder_config['seq_len']=kwargs['seq_len']
...
def vqnsp_encoder_base_decoder_3x200x12(pretrained=False, pretrained_weight=None, as_tokenzer=False, EEG_size=1600, 
    ...
    if as_tokenzer:
        ...
        for k in keys:
            if k.startswith("loss") or k.startswith("teacher") or k.startswith("scaling") or 'encoder.time_embed' in k:
                del weights[k]
